...
import 'package:seriesradar_app/features/serie/presentation/providers/season/season_repository_provider.dart';

final seasonInfoProvider =
    StateNotifierProvider<SeasonMapNotifier, Map<String, SeasonDetails>>((ref) {
...
typedef GetSeasonCallback = Future<SeasonDetails> Function(
    int serieId, int seasonNumber);
...
class SeasonMapNotifier extends StateNotifier<Map<String, SeasonDetails>> {
  ...
  Future<void> loadSeason(int serieId, int seasonNumber) async {
    final key =
        "$serieId-$seasonNumber"; // Usamos un identificador único para cada temporada.
