...
final recommendationsSeriesProvider = FutureProvider.family((ref, int serieId) {
  final serieRepository = ref.watch(serieRepositoryProvider);
  return serieRepository.getRecommendationsSeries(serieId);
});

class RecommendationsSeries extends ConsumerWidget {
  final int serieId;

  const RecommendationsSeries({super.key, required this.serieId});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final recommendationsFuture =
        ref.watch(recommendationsSeriesProvider(serieId));

    return recommendationsFuture.when(
      data: (series) {
        // Solo mostrar si hay series recomendadas
        if (series.isEmpty) {
...
class _Recomendations extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: SeriesHorizontalListview(
        series: series,
        height: 310,
      ),
    );
